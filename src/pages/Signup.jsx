...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.warn("Backend API offline. Creating mock registration record.", error);
      localStorage.setItem('token', 'mock-token-123');
      localStorage.setItem('user_email', formData.email);
      localStorage.setItem('user_name', formData.name);
      navigate('/dashboard');
  ...
  const handleGoogleLoginClick = () => {
    const finalName = formData.name.trim() || "Google Student";
    const finalEmail = formData.email.trim() || "student@gmail.com";
